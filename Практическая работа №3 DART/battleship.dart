...
void saveStatistics(int shotsFired, int shipsSunk, int hits, int misses, int shipsRemaining) {
  Directory('game_statistics').createSync();

  File('game_statistics/statistics.txt').writeAsStringSync('''
Игрок1:
- Разрушено кораблей противника: $shipsSunk
- Потеряно кораблей: ${shipSizes.length - shipsRemaining}
- Осталось кораблей на поле: $shipsRemaining
- Попаданий: $hits
- Промахов: $misses
- Всего выстрелов: $shotsFired
''');
}
...
void main() {
  ...
  saveStatistics(shotsFired, shipsSunk, playerHits, playerMisses, playerShipsRemaining);
}